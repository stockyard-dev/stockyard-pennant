...
async function load(){var r=await fetch(A+'/members').then(function(r){return r.json()});items=r.members||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.name||'').toLowerCase().includes(q)||(i.email||'').toLowerCase().includes(q)||(i.points||'').toLowerCase().includes(q)||(i.tier||'').toLowerCase().includes(q)});
...
if(i.points)h+='<span>'+esc(i.points)+'</span>';
if(i.tier)h+='<span>'+esc(i.tier)+'</span>';
if(i.total_earned)h+='<span>'+esc(i.total_earned)+'</span>';
if(i.total_redeemed)h+='<span>'+esc(i.total_redeemed)+'</span>';
if(i.joined_at)h+='<span>'+esc(i.joined_at)+'</span>';
h+='<span>'+ft(i.created_at)+'</span></div>';

h+='</div>';});document.getElementById('list').innerHTML=h;}
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/members/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{name:"",email:"",points:"",tier:"",total_earned:"",total_redeemed:"",joined_at:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' PENNANT</h2>';
...
h+='<div class="fr"><label>Points</label><input id="f-points" value="'+esc(i.points)+'"></div>';
h+='<div class="fr"><label>Tier</label><input id="f-tier" value="'+esc(i.tier)+'"></div>';
h+='<div class="fr"><label>Total Earned</label><input id="f-total-earned" value="'+esc(i.total_earned)+'"></div>';
h+='<div class="fr"><label>Total Redeemed</label><input id="f-total-redeemed" value="'+esc(i.total_redeemed)+'"></div>';
h+='<div class="fr"><label>Joined At</label><input id="f-joined-at" value="'+esc(i.joined_at)+'"></div>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),email:document.getElementById('f-email').value.trim(),points:document.getElementById('f-points').value.trim(),tier:document.getElementById('f-tier').value.trim(),total_earned:document.getElementById('f-total-earned').value.trim(),total_redeemed:document.getElementById('f-total-redeemed').value.trim(),joined_at:document.getElementById('f-joined-at').value.trim()};
if(editId){await fetch(A+'/members/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/members',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
